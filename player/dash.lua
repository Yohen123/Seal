...
function PlayerDash:init_player_dash()
...
function PlayerDash:set_aim_position(x, y)
...
function PlayerDash:can_dash()
    return not self.dashing and self.dash_cooldown_time == 0
end

function PlayerDash:start_dash()
...
function PlayerDash:get_dash_travel_progress()
    return math.max(0, math.min(1,
        (self.dash_time - self.dash_windup) / self.dash_travel))
end

function PlayerDash:get_dash_eased_progress()
    return ease_out_cubic(self:get_dash_travel_progress())
end

function PlayerDash:update_dash(dt)

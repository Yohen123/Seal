...
function Physics:wander(radius, distance, jitter, weight)
    self.wander_x = self.wander_x + (love.math.random() * 2 - 1) * (jitter or 20)
    self.wander_y = self.wander_y + (love.math.random() * 2 - 1) * (jitter or 20)
...
function Physics:rotate_towards_velocity(value)
    ...
    self.r = self.r + difference * math.max(0, math.min(value, 1))
end
